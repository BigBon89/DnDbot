...
public class Encounter {    private boolean inEncount;
    ...
    public Encounter() {        inEncount = false;
        monsters = new Monsters();    }
    public Encounter(int seed) {
        inEncount = false;        monsters = new Monsters(seed);
    }
    ...
    public String start(EncounterDifficulty difficulty,
        ...
        if (inEncount) {            return "The encounter has already started";
        }
        if (playersCount <= 0) {            return "Invalid players count";
        }
        if(!Arrays.asList(allowedMonsterTypes).contains(monsterFilter) && !monsterFilter.isEmpty()){            return "Invalid monster filter";
        }
        monsters.generate(difficulty, playersCount, playersLevel, monsterFilter);
        inEncount = true;        return printMonsters();
    }
    public String end() {        if (!inEncount) {
        return "The encounter has not started";        }
        inEncount = false;
        return "The encounter is over";    }
    public String attack(int monsterIndex, int damage) {
        if (!inEncount) {            return "The encounter has not started";
        }
        if (damage <= 0) {            return "Damage must be a positive number";
        }
        if (!monsters.isValidIndex(monsterIndex)) {            return "Wrong monster index";
        }
        monsters.damage(monsterIndex, damage);
        if (monsters.getMonstersCount() != 0) {            return printMonsters();
        }
